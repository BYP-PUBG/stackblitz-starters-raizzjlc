...
const TIERS=[{name:'Classic',color:'#888780',poly:8,priceMin:5,priceMax:15,guns:['Silver Plate - AKM','Silver Plate - M416','Silver Plate - SCAR-L','Silver Plate - SKS','Silver Plate - AWM','Silver Plate - DP-28','Silver Plate - S12K','Silver Plate - S1897']},{name:'Special',color:'#639922',poly:16,priceMin:20,priceMax:50,guns:['Jungle Digital - M16A4','Jungle Digital - SKS','Gold Plate - AKM','Gold Plate - M416','Gold Plate - AWM','Gold Plate - SKS']},{name:'Rare',color:'#378ADD',poly:28,priceMin:60,priceMax:150,guns:['Gunsmith Cobalt - QBU','Gunsmith Cobalt - P1911','Gunsmith Crimson - S12K','Gunsmith Crimson - Win94','Lucky Knight - M24']},{name:'Elite',color:'#7F77DD',poly:40,priceMin:150,priceMax:400,guns:['Neon - AKM','Neon - M416','Neon - SKS','Neon - AWM']},{name:'Epic',color:'#D4537E',poly:200,priceMin:500,priceMax:2000,guns:['Dragon Breath - AKM','Dragon Breath - M416','Infernal Dragon - SKS']},{name:'Legendary',color:'#E24B4A',poly:800,priceMin:2000,priceMax:8000,guns:['Lucky Knight - AKM','Lucky Knight - M416','Lucky Knight - AWM']},{name:'Ultimate',color:'#BA7517',poly:3600,priceMin:10000,priceMax:50000,guns:['Chroma - AKM','Chroma - M416']}]
            ...
            {TIERS.map(t=>{
              ...
              return(
                <div key={t.name} onClick={()=>openPopup(t)} className="bg-gray-800 rounded-xl p-3 cursor-pointer hover:-translate-y-0.5 transition-transform" style={{borderLeft:`3px solid ${t.color}`}}>
                  ...
                  <div className="text-sm font-bold text-white mt-2">{isZero?'0 อัน':`${gunsNeeded.toLocaleString()} อัน`}</div>
                  <div className="text-xs text-gray-400 mt-1">{isZero?'ครบแล้ว!':`฿${costMin.toLocaleString()} – ฿${costMax.toLocaleString()}`}</div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center px-4 z-50" onClick={()=>setPopup(null)}>
          <div className="bg-gray-900 rounded-2xl p-6 w-full max-w-sm" style={{border:`2px solid ${popup.color}`}} onClick={e=>e.stopPropagation()}>
            <div className="flex justify-between items-center mb-4">
              <div><h2 className="text-base font-bold" style={{color:popup.color}}>{popup.name}</h2><p className="text-xs text-gray-400 mt-1">{popup.poly} Polymers ต่ออัน</p></div>
            ...
            {!loadingPrice&&marketData&&(
              <div className="flex flex-col gap-3">
                <div className="grid grid-cols-2 gap-2">
                  ...
                  <div className="bg-gray-800 rounded-xl p-3 text-center"><div className="text-xs text-gray-400">ราคากลาง</div><div className="text-lg font-bold text-blue-400 mt-1">{marketData.median_price||'N/A'}</div></div>
                </div>
                <div className="bg-gray-800 rounded-xl p-3 text-center"><div className="text-xs text-gray-400">จำนวนที่วางขายอยู่</div><div className="text-lg font-bold text-white mt-1">{parseInt(marketData.volume||'0').toLocaleString()} ชิ้น</div></div>
