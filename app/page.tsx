...
export default function Home(){
  ...
  async function openPopup(tier:typeof TIERS[0]){
    ...
    setGunPrices(results);setLoadingGuns(false)
  }
  ...
  async function compareAll(){
    ...
    for(const tier of TIERS){
      try{
        ...
        const data=await res.json()
        results.push({name:tier.guns[0],price:parsePrice(data.lowest_price),volume:parseInt(data.volume||'0'),tier})
      }catch{results.push({name:tier.guns[0],price:null,volume:0,tier})}
    }
    setCompareResults(results);setLoadingCompare(false)
  }

  const valid=[...compareResults].filter(r=>r.price&&r.price>0)
  const byValue=[...valid].sort((a,b)=>(b.tier.poly/(b.price||999))-(a.tier.poly/(a.price||999)))
  const best=byValue[0]
  const second=byValue[1]

  function s1(){if(!best)return null;const qty=Math.ceil(needPoly/best.tier.poly);const cost=qty*(best.price||0);return{tier1:best,qty1:qty,cost,ppb:needPoly/cost}}
  function mix(r:number){
    ...
    return{tier1:best,tier2:second,qty1:q1,qty2:q2,totalQty:q1+q2,cost,ppb:needPoly/cost}
  }
    ...
    <div style={{maxWidth:'720px',margin:'0 auto',padding:'24px 16px'}}>
      ...
      <div style={{marginBottom:'24px'}}>
        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:'8px'}}>
          <h1 style={{fontSize:'28px',fontWeight:'800',color:'white',margin:0,letterSpacing:'-0.02em'}}>
            PUBG <span style={{color:'#E24B4A'}}>POLYMER</span>
          </h1>
          <div style={{display:'flex',gap:'6px'}}>
            {LANGS.map(l=>(
              <button key={l.code} onClick={()=>{setLang(l);setCompareResults([]);setComparing(false)}}
                style={{fontSize:'12px',padding:'6px 12px',borderRadius:'20px',border:'none',cursor:'pointer',fontWeight:'600',background:lang.code===l.code?'#E24B4A':'#1a1a2e',color:lang.code===l.code?'white':'#888',transition:'all 0.2s'}}>
                {l.label}
              </button>
            ))}
          </div>
      ...
      <div style={{...card,marginBottom:'16px'}}>
        ...
        <div style={{display:'flex',flexDirection:'column',gap:'12px'}}>
          <div style={{display:'flex',alignItems:'center',gap:'12px'}}>
            <label style={{fontSize:'13px',color:'#888',minWidth:'160px'}}>{t.currentLv}</label>
            <select value={currentLevel} onChange={e=>setCurrentLevel(parseInt(e.target.value))} style={{...input}}>
              {levels.filter(l=>l<10).map(l=>(<option key={l} value={l}>Level {l}</option>))}
            </select>
          </div>
          <div style={{display:'flex',alignItems:'center',gap:'12px'}}>
            <label style={{fontSize:'13px',color:'#888',minWidth:'160px'}}>{t.targetLv}</label>
            <select value={targetLevel} onChange={e=>setTargetLevel(parseInt(e.target.value))} style={{...input}}>
              {levels.filter(l=>l>currentLevel).map(l=>(<option key={l} value={l}>Level {l}</option>))}
            </select>
          </div>
          <div style={{display:'flex',alignItems:'center',gap:'12px'}}>
            <label style={{fontSize:'13px',color:'#888',minWidth:'160px'}}>{t.owned}</label>
            <input type="number" value={ownedPoly} min={0} onChange={e=>setOwnedPoly(e.target.value===''?'':parseInt(e.target.value))} placeholder="0" style={{...input}}/>
          </div>
        </div>
      </div>
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:'12px',marginBottom:'16px'}}>
        {[{label:t.needed,val:totalPolyNeeded.toLocaleString(),color:'white'},{label:t.lacking,val:needPoly.toLocaleString(),color:'#E24B4A'},{label:t.bp,val:totalBpNeeded.toString(),color:'white'}].map(s=>(
          <div key={s.label} style={{...cardSm,textAlign:'center'}}>
            <div style={{fontSize:'11px',color:'#666',marginBottom:'6px',letterSpacing:'0.05em'}}>{s.label}</div>
          ...
          <div style={{marginTop:'16px',display:'flex',flexDirection:'column',gap:'10px'}}>
            {rs1&&(
              <div style={{background:'rgba(99,153,34,0.1)',border:'1px solid rgba(99,153,34,0.3)',borderRadius:'12px',padding:'14px'}}>
                ...
                <div style={{fontSize:'12px',color:'#aaa',marginBottom:'10px'}}>ซื้อ <span style={{color:rs1.tier1.tier.color,fontWeight:'600'}}>{rs1.tier1.tier.name}</span> {rs1.qty1} {t.pcs}</div>
                <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:'8px',textAlign:'center'}}>
                  ...
                  <div><div style={{fontSize:'11px',color:'#555'}}>{t.qty}</div><div style={{fontSize:'15px',fontWeight:'700',color:'white'}}>{rs1.qty1} {t.pcs}</div></div>
            ...
            {rs2&&(
              <div style={{background:'rgba(55,138,221,0.1)',border:'1px solid rgba(55,138,221,0.3)',borderRadius:'12px',padding:'14px'}}>
                ...
                <div style={{fontSize:'12px',color:'#aaa',marginBottom:'10px'}}><span style={{color:rs2.tier1.tier.color,fontWeight:'600'}}>{rs2.tier1.tier.name}</span> {rs2.qty1} + <span style={{color:rs2.tier2.tier.color,fontWeight:'600'}}>{rs2.tier2.tier.name}</span> {rs2.qty2} {t.pcs}</div>
                <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:'8px',textAlign:'center'}}>
                  ...
                  <div><div style={{fontSize:'11px',color:'#555'}}>{t.qty}</div><div style={{fontSize:'15px',fontWeight:'700',color:'white'}}>{rs2.totalQty} {t.pcs}</div></div>
            ...
            {rs3&&(
              <div style={{background:'rgba(127,119,221,0.1)',border:'1px solid rgba(127,119,221,0.3)',borderRadius:'12px',padding:'14px'}}>
                ...
                <div style={{fontSize:'12px',color:'#aaa',marginBottom:'10px'}}><span style={{color:rs3.tier1.tier.color,fontWeight:'600'}}>{rs3.tier1.tier.name}</span> {rs3.qty1} + <span style={{color:rs3.tier2.tier.color,fontWeight:'600'}}>{rs3.tier2.tier.name}</span> {rs3.qty2} {t.pcs}</div>
                <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:'8px',textAlign:'center'}}>
                  ...
                  <div><div style={{fontSize:'11px',color:'#555'}}>{t.qty}</div><div style={{fontSize:'15px',fontWeight:'700',color:'white'}}>{rs3.totalQty} {t.pcs}</div></div>
      ...
      <div style={{...card,marginBottom:'16px'}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(2,1fr)',gap:'10px'}}>
          {TIERS.map(tr=>{
            ...
            return(
              <div key={tr.name} onClick={()=>openPopup(tr)} style={{background:'#1a1a2e',borderRadius:'12px',padding:'14px',cursor:'pointer',borderLeft:`3px solid ${tr.color}`,transition:'all 0.2s'}}>
      ...
      <div style={{...card}}>
        ...
        <div style={{display:'flex',flexDirection:'column',gap:'0'}}>
          {UPGRADES.filter(u=>u.level>currentLevel&&u.level<=targetLevel).map((u,idx)=>{
        ...
        <div onClick={()=>setPopup(null)} style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.8)',display:'flex',alignItems:'center',justifyContent:'center',padding:'16px',zIndex:200}}>
          <div onClick={e=>e.stopPropagation()} style={{background:'#0f0f1a',border:`1px solid ${popup.color}`,borderRadius:'20px',padding:'24px',width:'100%',maxWidth:'400px',maxHeight:'80vh',overflowY:'auto'}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'16px'}}>
              ...
              <button onClick={()=>setPopup(null)} style={{background:'#1a1a2e',border:'none',color:'#888',fontSize:'18px',width:'32px',height:'32px',borderRadius:'8px',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center'}}>×</button>
            </div>
              ...
              <div style={{display:'flex',flexDirection:'column',gap:'8px'}}>
                {gunPrices.map((g,idx)=>(
                  <div key={g.name} onClick={()=>setSelectedGun(g.name)}
                    style={{background:selectedGun===g.name?'#2a2a3e':'#1a1a2e',borderRadius:'12px',padding:'12px 14px',display:'flex',alignItems:'center',justifyContent:'space-between',cursor:'pointer',border:idx===0?`1px solid ${popup.color}`:'1px solid transparent',transition:'all 0.2s'}}>
            ...
            {selectedGun&&gunPrices.find(g=>g.name===selectedGun)?.price&&(
              <div style={{background:'#1a1a2e',borderRadius:'12px',padding:'14px',marginTop:'12px'}}>
                ...
                <div style={{fontSize:'13px',color:'white',marginBottom:'8px'}}>{t.buy} {Math.ceil(needPoly/popup.poly).toLocaleString()} {t.pcs} · {t.total} <span style={{color:'#4ade80',fontWeight:'700'}}>{fmt(Math.ceil(needPoly/popup.poly)*(gunPrices.find(g=>g.name===selectedGun)?.price||0))}</span></div>
                <a href={`https://steamcommunity.com/market/listings/578080/${encodeURIComponent(selectedGun)}`} target="_blank" rel="noopener noreferrer" style={{fontSize:'12px',color:'#378ADD',textDecoration:'none'}}>{t.market}</a>
              </div>
            )}
          </div>
        </div>
